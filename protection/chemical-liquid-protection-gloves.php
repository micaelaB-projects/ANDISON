<?php
$page_title = "Chemical and Liquid Protection Gloves";
?>
<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width,initial-scale=1">
  <title><?php echo $page_title; ?> - Andison Industrial</title>
  <style>
    body { font-family: Arial, Helvetica, sans-serif; padding: 28px; color:#222 }
    a { color:#2B11DB; text-decoration:none }
    .container { max-width: 980px; margin: 0 auto }
    h1 { color:#2B11DB }
    .products { margin-top:18px }
    .prod { padding:12px 0; border-bottom:1px solid #eee }
  </style>
</head>
<body>
  <div class="container">
    <a href="working-gloves.php">← Back to Working Gloves</a>
    <h1><?php echo $page_title; ?></h1>
    <p>Protective gloves designed to shield against chemical and liquid hazards in industrial environments.</p>
    <div class="products">
      <div class="prod">No products yet — add product blocks or include a catalog.</div>
    </div>
  </div>
</body>
</html>

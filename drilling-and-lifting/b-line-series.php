...
// Find the parent category and the sub-subcategory (nested under magnetic-drill)
foreach ($categories as $cat) {
    if ($cat['id'] === 'drilling-and-lifting') {
        $parent_category = $cat;
        if (!empty($cat['subcategories'])) {
            foreach ($cat['subcategories'] as $subcat) {
                if ($subcat['id'] === 'magnetic-drill' && !empty($subcat['subcategories'])) {
                    foreach ($subcat['subcategories'] as $sub_subcat) {
                        if ($sub_subcat['id'] === $subcategory_id) {
                            $current_category = $sub_subcat;
...
if (!$current_category) {
    die("B-Line Series category not found");
}

$page_title = $current_category['name'] ?? "B-Line Series";
...
<head>
    ...
    <?php 
    $category_name = $current_category['name'] ?? 'B-Line Series';
    $category_description = $current_category['description'] ?? 'Professional B-Line series drilling equipment for industrial applications.';
    ?>
...
<body>
    ...
    <div id="miniPopover" class="mini-popover" aria-hidden="true">
        ...
        <div class="category-content">
            <h2><?php echo htmlspecialchars($current_category['name'] ?? 'B-Line Series'); ?></h2>
            ...
            <div class="product-grid">
                <?php 
                // Dynamically fetch products for B-Line Series subcategory
                $all_products = array();
                ...
                if (!empty($all_products)) {
                    foreach ($all_products as $product) {
                        ...
                        $type = htmlspecialchars($product['type'] ?? 'Drilling Equipment');
                ...
                <div class="product-card">
                    ...
                    <h4>B-Line Series Equipment</h4>
                    <p class="product-type">No products available</p>
                    <button class="add-to-inquiry" type="button" data-model="B-Line Series" data-type="B-Line Series Equipment" data-brand="Industrial">ADD TO INQUIRY</button>
                </div>
                    <?php
                }
                ?>

...
// Detect category and subcategory from file path
$current_path = str_replace($_SERVER['DOCUMENT_ROOT'], '', $_SERVER['PHP_SELF']);
$current_path = str_replace('\\', '/', $current_path);
$path_parts = explode('/', trim($current_path, '/'));

// Extract category (e.g., drilling-and-lifting) and subcategory (e.g., magnetic-drill, b-line-series)
$category_id = '';
$subcategory_id = '';
$nested_subcategory_id = '';

if (($andison_idx = array_search('ANDISON', $path_parts)) !== false) {
    if (isset($path_parts[$andison_idx + 1])) {
        $category_id = $path_parts[$andison_idx + 1];
    }
    // Handle nested paths like drilling-and-lifting/magnetic-drill/b-line-series.php
    if (isset($path_parts[$andison_idx + 2]) && $path_parts[$andison_idx + 2] !== basename($_SERVER['PHP_SELF'])) {
        $subcategory_id = $path_parts[$andison_idx + 2];
        // Check for 3rd level nested subcategory (e.g., b-line-series under magnetic-drill)
        if (isset($path_parts[$andison_idx + 3]) && $path_parts[$andison_idx + 3] !== basename($_SERVER['PHP_SELF'])) {
            $nested_subcategory_id = $path_parts[$andison_idx + 3];
        }
    } else {
        // For non-nested, use the filename without .php extension
        $filename = basename($_SERVER['PHP_SELF']);
        $nested_subcategory_id = str_replace('.php', '', $filename);
    }
}

// Load all categories and find the matching ones
$categories = andison_get_categories();
$current_category = null;
$current_subcategory_info = null;
$current_nested_info = null;
    ...
    if ($cat['id'] === $category_id) {
        ...
        if (isset($cat['subcategories'])) {
            foreach ($cat['subcategories'] as $subcat) {
                // Check if this is the direct subcategory
                if ($subcategory_id && $subcat['id'] === $subcategory_id) {
                    $current_subcategory_info = $subcat;
                    
                    // If there's a nested level, look for it within this subcategory
                    if ($nested_subcategory_id && isset($subcat['subcategories'])) {
                        foreach ($subcat['subcategories'] as $nested) {
                            if ($nested['id'] === $nested_subcategory_id) {
                                $current_nested_info = $nested;
                                break;
                            }
                        }
                    }
                ...
                // If no direct subcategory match, check if the nested ID is in this subcat's children
                else if (!$subcategory_id && $nested_subcategory_id && isset($subcat['subcategories'])) {
                    foreach ($subcat['subcategories'] as $nested) {
                        if ($nested['id'] === $nested_subcategory_id) {
                            $current_subcategory_info = $subcat;
                            $current_nested_info = $nested;
                ...
                // Direct match to subcategory without nesting
                else if (!$subcategory_id && $subcat['id'] === $nested_subcategory_id) {
                    $current_subcategory_info = $subcat;
                    break;
                }
            }
...
// Determine which title to use (nested if available, otherwise subcategory)
$page_title = '';
if ($current_nested_info) {
    $page_title = $current_nested_info['name'];
    // For products, use nested subcategory ID if available
    $product_category = $category_id;
    $product_subcategory = $current_nested_info['id'];
} else if ($current_subcategory_info) {
    $page_title = $current_subcategory_info['name'];
    $product_category = $category_id;
    $product_subcategory = $current_subcategory_info['id'];
} else {
...
// Get products
$products = andison_get_products_for_subcategory($product_category, $product_subcategory);

// Get category description from the main category or a better fallback
...
<head>
    ...
    <title><?php echo htmlspecialchars($page_title ?? 'Products'); ?> - ANDISON INDUSTRIAL</title>
    ...
    <header>
        ...
        <div class="category-content">
            ...
            <div class="main-product-area">
                ...
                <?php 
                // Use the products already loaded at the top of the file

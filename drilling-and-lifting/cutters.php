...
// Extract category (e.g., drilling-and-lifting) and subcategory (e.g., cutters)
$category_id = '';
$subcategory_id = '';
...
if (($andison_idx = array_search('ANDISON', $path_parts)) !== false) {
    ...
    // For cutters.php at drilling-and-lifting level, use filename
    if (isset($path_parts[$andison_idx + 2]) && $path_parts[$andison_idx + 2] !== basename($_SERVER['PHP_SELF'])) {
        ...
        // For non-nested pages, use the filename without .php extension
        $filename = basename($_SERVER['PHP_SELF']);
        $subcategory_id = str_replace('.php', '', $filename);
    }
}
...
if (!$current_subcategory_info) {
    $current_subcategory_info = array(
        'id' => $subcategory_id,
        'name' => ucwords(str_replace('-', ' ', $subcategory_id))
    );
}
